...
const GlobalResult = () => {
  ...
  useEffect(() => {
    const fetchResult = async () => {
      ...
      setIsLoading(true)
      try {
        // make a call to database for EVERYTHING EVERYWHERE AT ONCE
            ...
            <div className="flex flex-col gap-2">
              {result.length > 0 ? (
                result.map((item:any, index: number) => (
                  <Link href={ renderLink('type', "id")} key={item.type + item.id + index} className="flex w-full cursor-pointer items-start gap-3 px-5 py-2.5 hover:bg-light-700/50 dark:bg-dark-500/50">
                    <Image
